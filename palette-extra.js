window.addEventListener('DOMContentLoaded',()=>{
  ...
  // Background: hsl(242 83% 9%) ≈ #05042a.
  const DEFAULT_BG='#05042a';
  const PALETTE={
    bg:DEFAULT_BG,
    proof:'#b72f7b',
    debate:'#176979',
  ...
  const LEGACY_BACKGROUNDS=['#030059','#061437','#030b24','#010615'];
  ...
  if(changed){localStorage.setItem('chooseLoserSettings',JSON.stringify(settings));applySettings();}
  ...
  window.openSettings=function(){
    ...
    const opacity=document.getElementById('opacitySetting');const opacityValue=document.getElementById('opacitySettingValue');
    if(opacity)opacity.value=Math.round(settings.opacity*100);if(opacityValue)opacityValue.textContent=Math.round(settings.opacity*100);
  };
});